...
- `Refiner` 当前有 Prompt 模板，但默认 invoke 是 no-op，不真正调用 LLM。
...
注意：当前默认 Refiner invoke 是 no-op，实际返回 `{}`，不调用 LLM。以下是已定义的 Prompt 模板，供后续替换真实 Refiner 使用。
...
| Refiner | 修正 Prompt 模板 | 默认 invoke no-op，不实际调用 LLM |
