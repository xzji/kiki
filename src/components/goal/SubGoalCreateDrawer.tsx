    ...
    <>
      ...
      <aside className="fixed right-0 top-0 z-50 flex h-screen w-[420px] flex-col border-l border-[#E5E7EB] bg-white">
        <div className="flex items-center justify-between border-b border-[#E5E7EB] px-5 py-4">
          <h3 className="text-sm font-semibold text-[#1F2328]">添加子目标</h3>
          <button type="button" className="rounded-md p-1.5 text-[#6B7280] hover:bg-[#F5F6F8]" onClick={onClose} aria-label="关闭">
            <X className="h-4 w-4" />
          </button>
        </div>
        <div className="flex-1 overflow-y-auto px-5 py-5">
          <label className="text-xs font-medium text-[#6B7280]">子目标标题</label>
          <input
            autoFocus
            value={title}
          ...
          />
          <p className="mt-3 text-xs leading-5 text-[#8C9198]">子目标创建后，你可以继续在里面添加具体任务。</p>
        </div>
