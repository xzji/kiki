...
export const ShareCard = forwardRef<HTMLDivElement, { question: string; answer: string }>(
  function ShareCard({ question, answer }, ref) {
    return (
      <div ref={ref} className="w-[760px] bg-surface-subtle px-12 pb-9 pt-11">
        <div className="rounded-[28px] border-2 border-line bg-white/60 p-8">
          {question.trim() ? (
            <div className="flex items-start justify-end gap-3">
              <div className="max-w-[520px] rounded-2xl rounded-br-sm bg-[#111] px-4 py-2.5 text-sm leading-6 text-white">
                {question}
              </div>
              <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-brand-soft/25 bg-brand-surface text-[13px] text-brand-soft">
                J
              </div>
            </div>
          ) : null}
        ...
        <div className="mt-5 flex items-center justify-end gap-2.5 text-right">
          ...
          <div className="leading-tight">
            <div className="text-[13px] font-semibold text-ink-soft">生成自KiKi Agent</div>
