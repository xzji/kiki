...
export function ExecutionResultBody(props: {
  ...
  const resultBlock = (
    <div>
      <div className="mb-3 text-[13px] font-medium text-[#1F2328]">执行结果</div>
      ...
      {currentKind === "confirm_action" && instance.payload.kind === "confirm_action" ? (
        <ConfirmActionView summary={instance.payload.summary} onConfirm={finish} onRevise={finish} />
      ) : null}
      {currentKind === "draft_review" && instance.payload.kind === "draft_review" ? (
        <DraftReviewView drafts={instance.payload.drafts} onComplete={finish} onRewrite={finish} />
      ) : null}
      ...
      {instance.awaitingUser ? (
        <div className="mt-4 rounded-xl border border-[#F5D58B] bg-[#FFF9E8] p-4 text-[13px] leading-6 text-[#6E5A16]">
          <div className="font-medium text-[#8A6D3B]">{awaitingUserPanelTitle(instance)}</div>
  ...
  const timelineBlock = (
    <details className="rounded-xl border border-[#E5E7EB] bg-white" open={instance.notification?.detailPolicy.showTimelineByDefault || instance.status === "in_progress"}>
      ...
      <div className="border-t border-[#E5E7EB] p-4">
        <TaskExecutionTimeline steps={instance.timeline ?? []} />
      </div>
    </details>
  );
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <div className="mt-1 flex flex-wrap items-center gap-2 text-[12px] text-[#8C9198]">
              ...
              <span className="text-[#D0D7DE]">/</span>
              <span>尝试 {instance.runner?.attemptCount ?? 0} 次</span>
            </div>
          </div>
...
function awaitingUserPanelTitle(instance: TaskInstance) {
  const type = instance.awaitingUser?.interactionRequirement?.type ?? instance.result?.interactionRequirement?.type;
