  ...
  return (
    <div className="px-2 py-2">
      <div className="mb-6 flex items-start justify-between gap-6">
        <div>
          <div className="mb-2 text-xs text-ink-soft"><Link href={`/goals/${goal.id}`} className="hover:text-[#111]">{goal.title}</Link> / {task.title.replace(/^任务\d+：/, "")}</div>
          <h1 className="text-[28px] font-semibold text-[#111]">{task.title.replace(/^任务\d+：/, "")}</h1>
          <p className="mt-2 text-sm text-ink-soft">完成进度：{task.progress}%（{task.expectedOutcome}）</p>
        </div>
        <Link href={`/goals/${goal.id}/tasks/${task.id}?view=${showDetail ? "list" : "detail"}`} className="rounded-lg border border-line-strong px-3 py-2 text-sm text-[#111] hover:bg-white">
          ...
          return (
            <div key={instance.id} className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="rounded-xl border border-ink-soft bg-surface p-4">
                  <div className="mb-2 flex items-center gap-2 text-sm font-medium text-[#111]">
                    ...
                    {isToday && canStart ? <span className="inline-flex h-2 w-2 rounded-full bg-badge" /> : null}
                  </div>
                  <p className="text-sm leading-6 text-ink-soft">{instance.intro}</p>
                  {isToday && canStart ? (
                    <div className="mt-4 flex justify-center">
                      <Link href={`/goals/${goal.id}/tasks/${task.id}?view=exec&instanceId=${instance.id}`} className="rounded-md border border-ink-soft px-5 py-1.5 text-xs text-[#111] hover:bg-surface">开始</Link>
                    </div>
                  ) : null}
                </div>
              </div>
