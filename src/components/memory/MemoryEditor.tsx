...
export function MemoryEditor({
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-4">
      ...
      <div className="rounded-2xl border border-line bg-surface-subtle px-4 py-3 text-[12px] text-ink-soft">
        Markdown 是记忆正文的唯一事实来源。自动提炼可能出错，你可以直接编辑或清空。当前上限：{limitLabel}。
      </div>

      <div className="inline-flex w-fit items-center gap-2 rounded-full border border-line bg-white px-3 py-1 text-[12px] text-ink-strong">
      ...
      <textarea
        ...
        disabled={loading}
        className="min-h-[280px] flex-1 resize-none rounded-2xl border border-line-strong bg-white px-4 py-3 font-mono text-[12px] leading-5 text-ink outline-none focus:border-[#111] disabled:bg-surface"
        placeholder={loading ? "正在读取记忆..." : "# Memory\n\n"}
      />

      <div className="flex items-center justify-between gap-3">
        <div className="min-h-5 text-[12px] text-ink-soft">{message}</div>
        <div className="flex items-center gap-2">
          <button
            type="button"
            onClick={() => void save("")}
            disabled={saving || loading || !content.trim()}
            className="rounded-md border border-danger-border px-3 py-1.5 text-[12px] text-danger-hover hover:bg-danger-bg disabled:cursor-not-allowed disabled:opacity-50"
          >
            清空
          </button>
          <button
            ...
            className="rounded-md border border-line-strong px-3 py-1.5 text-[12px] text-ink hover:border-[#111] disabled:cursor-not-allowed disabled:opacity-50"
          >
            刷新
          </button>
          <button
