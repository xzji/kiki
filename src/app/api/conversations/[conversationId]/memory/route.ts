...
async function GETHandler(_: NextRequest, context: { params: Promise<{ conversationId: string }> }) {
  const { conversationId } = await context.params;
  const memory = readSessionMemory(conversationId);
  return NextResponse.json({
    ok: true,
    memory,
    source: getLatestMemoryAuditSource({ target: "session", conversationId }) ?? "用户手动",
  });
}

async function PUTHandler(request: NextRequest, context: { params: Promise<{ conversationId: string }> }) {
  const { conversationId } = await context.params;
  ...
  if ("overLimit" in result && result.overLimit) {
    return NextResponse.json({ ok: false, reason: "会话记忆超过 6KB 上限", ...result }, { status: 413 });
  }
