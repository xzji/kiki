...
function resolveBlocker(blocker: ExecutionBlocker, input: Pick<RequestBody, "approved" | "feedback">): ExecutionBlocker {
  return {
    ...blocker,
    status: "resolved",
...
export async function POST(request: NextRequest) {
  ...
  if (!body.taskInstanceId || !body.resumeToken) {
    return NextResponse.json({ reason: "taskInstanceId 和 resumeToken 不能为空" }, { status: 400 });
  }
