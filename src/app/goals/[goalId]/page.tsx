...
import type { Task, TaskInstanceStatus } from "@/types/dora";
...
export default function GoalDetailPage({ params }: { params: { goalId: string } }) {
  ...
  const summary = useMemo(() => {
    ...
    const daysLeft = Math.max(0, Math.ceil((new Date(goal.deadline).getTime() - BASE_DATE.getTime()) / (1000 * 60 * 60 * 24)));
  ...
  return (
    <div className="max-w-[920px] pb-12">
      <div className="mb-7 text-xs text-[#8C9198]">
        进行中 <span className="mx-1">/</span> <span className="font-medium text-[#1F2328]">{goal.title}</span>
      </div>
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex flex-1 items-center gap-5">
            ...
            <div className="min-w-0 flex-1">
              <h1 className="text-[28px] font-semibold tracking-[-0.02em] text-[#1F2328]">{goal.title}</h1>
              <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-2 text-sm text-[#6B7280]">
                ...
                <span className="rounded-md bg-[#E9EEF5] px-2.5 py-1 text-xs font-medium text-[#1F2328]">剩余 {summary.daysLeft} 天</span>
                <span className="inline-flex items-center gap-1.5">
...
function SummaryStat({ label, value, muted = false }: { label: string; value: number; muted?: boolean }) {
  return (
    <div className="text-center">
      <div className={`text-[24px] font-semibold tracking-[-0.03em] ${muted ? "text-[#9AA0A6]" : "text-[#1F2328]"}`}>{value}</div>
