...
import { topicDetailPath } from "@/lib/routes";
...
export default function TopicDeliverablePage({ params }: { params: { topicId: string } }) {
  ...
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    fetch(`/api/topics/${encodeURIComponent(topicId)}/deliverable`, { cache: "no-store" })
      .then(async (response) => {
        const payload = (await response.json()) as { ok?: boolean; reason?: string; deliverable?: GoalDeliverable };
        if (!response.ok || !payload.deliverable) throw new Error(payload.reason || "主题交付包加载失败");
        if (!cancelled) {
      ...
      .catch((nextError) => {
        if (!cancelled) setError(nextError instanceof Error ? nextError.message : "主题交付包加载失败");
      })
      .finally(() => {
  ...
  return (
    <main className="max-w-[920px] pb-12">
      <div className="mb-4 text-right text-xs text-[#6B7280]">
        <Link href={topicDetailPath(topicId)} className="font-medium text-[#1F2328] hover:text-[#111]">
          返回主题规划
        </Link>
      </div>
      <section className="rounded-[20px] border border-[#E5E7EB] bg-white p-6">
        <div className="text-xs font-medium uppercase tracking-[0.18em] text-[#8C9198]">Topic Deliverable</div>
