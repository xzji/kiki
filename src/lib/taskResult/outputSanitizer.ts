...
function isRedundantOuterNarration(block: ResultBlock, outerTexts: string[]) {
  if (block.kind !== "callout" && block.kind !== "paragraph" && block.kind !== "markdown") return false;
  const text = normalizeForCompare(blockText(block));
  if (!text) return false;
  return outerTexts.some((outerText) => {
    const normalizedOuter = normalizeForCompare(outerText);
    if (!normalizedOuter) return false;
    return text === normalizedOuter || text.includes(normalizedOuter) || normalizedOuter.includes(text);
  });
}

export function sanitizeTaskResultOutput(
  taskResult: TaskResult,
  input: { outerTexts?: Array<string | null | undefined> } = {},
): TaskResult {
  const outerTexts = (input.outerTexts ?? []).filter((text): text is string => Boolean(text?.trim()));
  const blocks = taskResult.blocks.filter(
    (block) => !isUnsafeMetaNarration(block) && !isRedundantOuterNarration(block, outerTexts),
  );
